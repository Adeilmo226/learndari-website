...
/**
 * Level 3 Quiz: Simple Words
 * Show Dari word, user selects correct English translation
 */
export default function Level3QuizPage() {
  const { user } = useUser();
  ...
  useEffect(() => {
    if (quizComplete && !progressSaved) {
      ...
      if (percentage >= 80 && user?.id) {
        saveLevelProgress(user.id, "level-3", percentage, "level-4").then(() => {
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            {passed && (
              <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 mb-6">
                <h3 className="text-xl font-bold text-green-900 mb-2">🎓 Level 3 Complete!</h3>
                <p className="text-green-700">You've unlocked Level 4: Common Words</p>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {passed ? (
                <>
                  <Link
                    href="/learn/level-4"
                    className="px-8 py-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl hover:from-purple-600 hover:to-purple-700 transition-colors font-semibold"
                  >
                    Continue to Level 4 →
                  </Link>
                  <Link
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <h1 className="text-3xl font-bold text-gray-900">Simple Words Test</h1>
        ...
        {showResult && (
          <button
            onClick={handleNext}
            className="w-full px-8 py-4 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-semibold text-lg"
          >
...
function generateQuizQuestions(): QuizQuestion[] {
  ...
  return shuffled.map((word) => {
    const wrongAnswers = simpleWords
