...
export default function QuizPage() {
   ...
   */
  const playAudio = () => {
    const audioUrl = `/audio/weather/${currentQuestion.word.id}.mp3`;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {weatherVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
...
function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...weatherVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = weatherVocab
