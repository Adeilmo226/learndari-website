...
import { fruitsVocab, setInfo } from "../data";
...
export default function FlashcardsPage() {
  ...
  const currentWord = fruitsVocab[currentIndex];
  const progress = ((currentIndex + 1) / fruitsVocab.length) * 100;

  const nextCard = () => {
    if (currentIndex < fruitsVocab.length - 1) {
  ...
  const playAudio = () => {
    const audioUrl = `/audio/fruits/${currentWord.id}.mp3`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Card {currentIndex + 1} of {fruitsVocab.length}
              </p>
            </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex justify-between mt-2 text-sm text-gray-600">
            ...
            <span>{fruitsVocab.length - currentIndex - 1} remaining</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 mb-3">
          ...
          <button
            onClick={nextCard}
            disabled={currentIndex === fruitsVocab.length - 1}
            className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-gray-300 rounded-lg hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Next
        ...
        {currentIndex === fruitsVocab.length - 1 && (
          <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 text-center">
            <h3 className="text-2xl font-bold text-green-900 mb-2">
              Great job! 🎉
            </h3>
            <p className="text-green-700 mb-4">
              You've completed all {fruitsVocab.length} cards!
