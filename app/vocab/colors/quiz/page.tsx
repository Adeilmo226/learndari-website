...
export default function QuizPage() {
   ...
   */
  const playAudio = () => {
    const audioUrl = `/audio/colors/${currentQuestion.word.id}.mp3`;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {colorsVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
...
function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...colorsVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = colorsVocab
