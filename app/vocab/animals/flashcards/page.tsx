...
import { animalsVocab, setInfo } from "../data";
...
export default function FlashcardsPage() {
  ...
  const currentWord = animalsVocab[currentIndex];
  const progress = ((currentIndex + 1) / animalsVocab.length) * 100;

  const nextCard = () => {
    if (currentIndex < animalsVocab.length - 1) {
  ...
  const playAudio = () => {
    const audioUrl = `/audio/animals/${currentWord.id}.mp3`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Card {currentIndex + 1} of {animalsVocab.length}
              </p>
            </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex justify-end mt-2 text-sm text-gray-600">
            <span>{animalsVocab.length - currentIndex - 1} remaining</span>
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <h2 className="text-5xl font-bold text-white mb-2" dir="rtl">
                  {currentWord.dari}
                </h2>
                <p className="text-xl text-red-100 italic mb-3">
        ...
        <div className="flex items-center justify-between gap-4 mb-3">
          ...
          <button
            onClick={nextCard}
            disabled={currentIndex === animalsVocab.length - 1}
            className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-gray-300 rounded-lg hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Next
        ...
        {currentIndex === animalsVocab.length - 1 && (
          <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 text-center">
            <h3 className="text-2xl font-bold text-green-900 mb-2">
              Great job! 🎉
            </h3>
            <p className="text-green-700 mb-4">
              You've completed all {animalsVocab.length} cards!
            </p>
