...
export default function QuizPage() {
   ...
   */
  const playAudio = () => {
    const audioUrl = `/audio/animals/${currentQuestion.word.id}.mp3`;
    ...
    <div className="bg-gray-50 py-4">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-2xl font-bold text-gray-900 mb-1" dir="rtl">
                        {option}
                      </p>
                      <p className="text-gray-600">
                        {animalsVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
        ...
        {showResult && (
          <button
            onClick={handleNext}
            className="w-full px-8 py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-colors font-semibold text-lg"
          >
...
function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...animalsVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = animalsVocab
