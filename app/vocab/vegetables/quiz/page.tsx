...
export default function QuizPage() {
   ...
   */
  const playAudio = () => {
    const audioUrl = `/audio/vegetables/${currentQuestion.word.id}.mp3`;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {vegetablesVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
...
function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...vegetablesVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = vegetablesVocab
