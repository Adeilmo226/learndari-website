...
export default function QuizPage() {
   ...
   */
  const playAudio = () => {
    const audioUrl = `/audio/transportation/${currentQuestion.word.id}.mp3`;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {transportationVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
...
interface QuizQuestion {
  word: typeof transportationVocab[0];
  options: string[];
...
function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...transportationVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = transportationVocab
