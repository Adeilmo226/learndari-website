...
export default function QuizPage() {
  ...
  const playAudio = () => {
    const audioUrl = `/audio/numbers/${currentQuestion.word.id}.mp3`;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-600">
                        {numbersVocab.find((w) => w.dari === option)?.phonetic}
                      </p>
                    </div>
                    {showCorrect && (
...
interface QuizQuestion {
  word: typeof numbersVocab[0];
  options: string[];
  correctAnswer: string;
}

function generateQuizQuestions(): QuizQuestion[] {
  const shuffled = [...numbersVocab].sort(() => Math.random() - 0.5);

  return shuffled.map((word) => {
    const wrongAnswers = numbersVocab
